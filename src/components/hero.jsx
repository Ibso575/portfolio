    ...
    >
      ...
      <div className="relative justify-center max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 gap-12 items-center w-full z-20 pt-16">
        ...
        <div className="relative flex items-center justify-center h-[500px] w-full">
          <div className="text-9xl font-bold text-[#ff2b28]/20 select-none">
            &lt;/&gt;
          </div>
          <motion.div
            animate={{ rotate: 360 }}
            transition={{ duration: 30, repeat: Infinity, ease: "linear" }}
            // Markazda turishi uchun transform-translate-half qilingan
            className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] h-[400px] md:w-[480px] md:h-[480px] rounded-full "
          >
            {techIcons.map(({ Icon, color }, index) => {
              const angle = (index / techIcons.length) * 360;

              // Radiusni CSS variable yoki Tailwind responsive klasslari orqali boshqarish yaxshiroq
              // Lekin kodingizdagi mantiqni saqlab qolamiz:
              const radius = 210; // Asosiy radius

              return (
                <div
                  key={index}
                  className="absolute left-1/2 top-1/2"
                  // rotate(-angle) ichki ikonalarni tik holatda ushlab turadi
                  style={{
                ...
                >
                  <motion.div
                    ...
                    className="w-12 h-12 md:w-16 md:h-16 bg-gray-950  rounded-2xl flex items-center justify-center shadow-2xl"
                  >
                    <Icon className={`${color} text-2xl md:text-4xl`} />
                  </motion.div>
                </div>
              );
            })}
